 ...
 */
public class WeekKLineChartRender extends KLineChartBaseRender {
    ...
    public void drawXAxis(List<ChartController.ChartInfo> currentPageData, List<ChartController.ChartInfo> previousPageData, List<ChartController.ChartInfo> nextPageData, Canvas canvas) {
        ...
        if (previousPageData != null && !previousPageData.isEmpty()) {
            ...
            for (int j = 0; j < subPreChartInfo.size(); j++) {
                ChartController.ChartInfo data = subPreChartInfo.get(j);
                if (data.isDrawLine) {
        ...
        if (nextPageData != null && !nextPageData.isEmpty()) {
            ...
            for (int j = 0; j < subNextChartInfo.size(); j++) {
                ChartController.ChartInfo data = subNextChartInfo.get(j);
                if (data.isDrawLine) {
                    time = formatSecond(data.time, "yyyy-MM");
                    paint.getTextBounds(time, 0, time.length(), textBounds);
