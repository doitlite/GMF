 ...
 */
public class MonthKLineChartView extends KLineBaseView {


    public MonthKLineChartView(Context context) {
        this(context, null);
    }

    public MonthKLineChartView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MonthKLineChartView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void drawXAxis(Canvas canvas) {
        ...
        for (int i = 0; i < mKLineDatas.size(); i++) {
            String time = formatSecond(timeList.get(i), "MM-dd");
            paint.getTextBounds(time, 0, time.length(), textBounds);
            if (i != 0 && !formatSecond(timeList.get(i), "MM").equals(formatSecond(timeList.get(i - 1), "MM"))) {
